Bootstrap重采样技术在环境调查中的应用
========================================================

环境调查所得到的数据在可靠性上容易被质疑，这种质疑一方面来自于小样本数据中异常值的解释而另一方面则来自于数据分析所采用模型背后的假设。Bootstrap重采样技术在统计学领域常用来解决小样本无分布假设问题，具有良好的鲁棒性与预测性能。通过将样本看作总体进行有放回的多次重采样并进行模拟求解，在不清楚样本所符合的分布条件下可以给出可靠性更高的中位数95%置信区间估计值，这使得对现实环境污染物污染水平的描述更准确。此外，结合局部加权回归散点平滑法（LOESS算法）与可视化技术，采用Bootstrap重采样技术可更好的讨论环境变量间的关系，挖掘数据背后的规律。通过对EPA公开的普吉特海湾多氯联苯（PCBs）调查数据的分析，我们发现PCBs浓度随深度增加而增加的线性模型受采样点设置的影响，深度0~200米PCBs浓度随深度增加而增加，在300米附近浓度呈稳定趋势，而深度400之后的浓度波动较大。
